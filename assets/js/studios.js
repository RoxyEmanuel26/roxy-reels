  ...
  if (studiosList.length === 0) {
    ...
    grid.innerHTML = `
      <div class="empty-state" style="grid-column: 1 / -1; width: 100%;">
        ...
        <h3>Studio Tidak Ditemukan</h3>
        <p>Studio "${escapedQuery}" tidak ada di daftar lokal kami. Apakah Anda ingin mencari video dengan studio ini langsung di database server?</p>
        <button id="search-api-studio-btn" class="btn-primary" data-name="${encodeURIComponent(searchQuery)}">
          Cari Studio: "${escapedQuery}" di Server
        </button>
      </div>
    `;
...
export function init() {
  ...
  mainApp.innerHTML = `
    <div class="taxonomy-browse-header">
      <div>
        <h2 style="font-size: var(--text-lg); font-weight: 800; margin-bottom: var(--space-1);">🎬 Studio JAV Terpopuler</h2>
        <p class="text-muted" style="font-size: var(--text-xs); font-weight: 500;">Telusuri daftar rilis video eksklusif berdasarkan studio produksi favorit Anda</p>
      </div>
