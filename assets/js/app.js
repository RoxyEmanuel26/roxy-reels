...
import ui from './ui.js?v=2.8.50';
import { renderVideoCard, bindHoverPreviews } from './feed.js?v=2.8.50';
import i18n from './i18n.js?v=2.8.50';
import { Analytics } from './analytics.js?v=2.8.50';
import ReferralSystem from './referral.js?v=2.8.50';
import './ads.js?v=2.8.50';
...
const routes = {
  '/':          () => import('./feed.js?v=2.8.50').then(m => m.init()),
  '/trending':  () => import('./trending.js?v=2.8.50').then(m => m.init()),
  '/recent':    () => import('./recent.js?v=2.8.50').then(m => m.init()),
  '/search':    (q) => import('./search.js?v=2.8.50').then(m => m.init(q || getParam('q'))),
  '/watch':     (id) => import('./player.js?v=2.8.50').then(m => m.init(id || window.missavJGetCurrentWatchId())),
  '/category':  () => import('./feed.js?v=2.8.50').then(m => m.init({ category: getParam('name') })),
  '/actor':     () => import('./feed.js?v=2.8.50').then(m => m.init({ actor: getParam('name') })),
  '/studio':    () => import('./feed.js?v=2.8.50').then(m => m.init({ studio: getParam('name') })),
  '/tag':       () => import('./feed.js?v=2.8.50').then(m => m.init({ tag: getParam('name') })),
  
  // Taxonomy browsing routes for Actors, Studios & Categories
  '/actors':          () => import('./actors.js?v=2.8.50').then(m => m.init()),
  '/popular-actors':  () => import('./popular_actors.js?v=2.8.50').then(m => m.init()),
  '/studios':         () => import('./studios.js?v=2.8.50').then(m => m.init()),
  '/categories':      () => import('./categories.js?v=2.8.50').then(m => m.init()),
...
function navigate(urlPath) {
  ...
  if (prevPath === '/watch' && matchedRoutePath === '/watch' &&
    ...
    import('./player.js?v=2.8.50').then(m => {
