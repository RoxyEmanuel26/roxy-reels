...
import ui from './ui.js?v=2.4.3';
import { renderVideoCard, bindHoverPreviews } from './feed.js?v=2.4.3';
import i18n from './i18n.js?v=2.4.3';
import { Analytics } from './analytics.js?v=2.4.3';
import ReferralSystem from './referral.js?v=2.4.3';
import './ads.js?v=2.4.3';
...
const routes = {
  '/':          () => import('./feed.js?v=2.4.3').then(m => m.init()),
  '/trending':  () => import('./trending.js?v=2.4.3').then(m => m.init()),
  '/recent':    () => import('./recent.js?v=2.4.3').then(m => m.init()),
  '/search':    (q) => import('./search.js?v=2.4.3').then(m => m.init(q || getParam('q'))),
  '/watch':     (id) => import('./player.js?v=2.4.3').then(m => m.init(id || window.missavJGetCurrentWatchId())),
  '/category':  () => import('./feed.js?v=2.4.3').then(m => m.init({ category: getParam('name') })),
  '/actor':     () => import('./feed.js?v=2.4.3').then(m => m.init({ actor: getParam('name') })),
  '/studio':    () => import('./feed.js?v=2.4.3').then(m => m.init({ studio: getParam('name') })),
  '/tag':       () => import('./feed.js?v=2.4.3').then(m => m.init({ tag: getParam('name') })),
  
  // Taxonomy browsing routes for Actors, Studios & Categories
  '/actors':          () => import('./actors.js?v=2.4.3').then(m => m.init()),
  '/popular-actors':  () => import('./popular_actors.js?v=2.4.3').then(m => m.init()),
  '/studios':         () => import('./studios.js?v=2.4.3').then(m => m.init()),
  '/categories':      () => import('./categories.js?v=2.4.3').then(m => m.init()),
...
function navigate(urlPath) {
  ...
  // Reset Adsterra frequency caps setiap navigasi SPA agar tayangan iklan baru
  if (window.missavJAds && typeof window.missavJAds.clearAdsterraSession === 'function') {
    window.missavJAds.clearAdsterraSession();
  }

  // Manage Global Top Ad Visibility and Loading
  const globalTopAd = document.getElementById('top-global-ad');
  if (globalTopAd) {
    if (matchedRoutePath === '/watch') {
      globalTopAd.style.display = 'none';
      globalTopAd.innerHTML = ''; // Hancurkan iklan lama agar tidak memakan memori
    } else {
      globalTopAd.style.display = '';
      globalTopAd.innerHTML = ''; // Bersihkan dulu supaya lazy observer bisa attach ulang
      if (window.missavJAds && typeof window.missavJAds.loadGlobalTopAd === 'function') {
        window.missavJAds.loadGlobalTopAd('top-global-ad');
      }
    }
  }

  // Manage Sticky Bottom Mobile Ad
  if (window.missavJAds && typeof window.missavJAds.loadStickyBottomAd === 'function') {
    window.missavJAds.loadStickyBottomAd();
  }
  ...
  if (prevPath === '/watch' && matchedRoutePath === '/watch' &&
    ...
    import('./player.js?v=2.4.3').then(m => {
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  if (window.missavJAds) {
    if (typeof window.missavJAds.initAdsterraSocialBar === 'function') {
      window.missavJAds.initAdsterraSocialBar();
    }
    if (typeof window.missavJAds.initAdsterraPopunder === 'function') {
      window.missavJAds.initAdsterraPopunder();
    }
  }
