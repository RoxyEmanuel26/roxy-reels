...
- **FIXED (PR #4, branch fix/seo-title-meta-length)**: "Title too long" (148 actor pages), "Meta description too long" (168 actor pages), "Meta description too short" (5 hub pages). All served by the Worker listing branch in `functions/[[catchall]].js`. Added `truncateChars()` (clips RAW name, escape AFTER, so entities never split). Actor title = `{name<=49} | MISSAV-J` (<=60 chars); actor desc = `Watch JAV videos starring {name<=65} in premium HD on MISSAV-J. Explore the full filmography and profile.` (110-160). Hub pages (/trending /recent /actors /categories /studios) descriptions expanded into 110-160. Verified against all 148/168 flagged crawl URLs. NOTE: category/studio pages use the same `{safeName} ...` pattern and could exceed limits for long names in future crawls — not flagged 2026-07-27 so left untouched; apply the same truncateChars approach if they surface.
- **STILL OPEN** (not touched): Orphan pages (1,749 indexable), Non-canonical page in sitemap (2), IndexNow notice (1,750), plus minor redirect/sitemap notices.
